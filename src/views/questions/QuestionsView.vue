  ...
  <v-container class="mx-auto" style="max-width: 800px">
    <v-row class="ma-0">
      <v-col
        ...
        align-center
      >
        <h1 class="TitlePages noBreak">Questions {{ this.currentPage }}</h1>
      </v-col>
      <!-- Search -->
      <v-col
        cols="12"
            ...
            <v-tab-item v-for="i in 3" :key="i" :value="'tab-' + i">
              ...
              <v-row class="ma-0">
                <v-col cols="12" class="order-1 order-md-0 mx-auto px-0">
                  <QuestionComponent
                    v-for="(question, idx) in filtredQuestions"
                    :key="`${idx}-${currentPage}-${search}`"
  ...
  methods: {
    ...
    async fetchData() {
      try {
        ...
        this.GET_LIKES();
        this.GET_DISLIKES();
        this.allItems = await QuestionService.getQuestions();
